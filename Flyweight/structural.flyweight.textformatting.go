...
// Tener un arreglo para cada letra puede resultar una gran desperdicio de espacio
...
type BetterFormattedText struct {
	plainText string
	// Este arreglo solo requiere la posición inicial y final al que se le van a
	// aplicar el estilo del texto.
...
func main() {
	...
	// Esto es posible porque Range() nos retorna un apuntador, al que le podemos acceder a la variable
	// Capitaliza para asignarla como true
	bft.Range(16, 19).Capitalize = true // new
	// En este caso el método String() solo podría aplicar el Capitalize al string
