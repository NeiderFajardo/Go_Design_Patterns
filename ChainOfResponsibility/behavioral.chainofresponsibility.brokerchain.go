...
func NewIncreasedDefenseModifier(g *Game, c *Creature) *IncreaseDefenceModifier {
	m := &IncreaseDefenceModifier{CreatureModifier{g, c}}
	g.Subscribe(m)
	return m
}

func (m *IncreaseDefenceModifier) Handle(q *Query) {
	if m.creature.Name == q.CreatureName && q.WhatToQuery == Defense {
		q.Value += 1
	}
}

func (m *IncreaseDefenceModifier) Close() error {
...
func main() {
...

	{
		...
		p := NewIncreasedDefenseModifier(game, goblin)
		fmt.Println(goblin.String())
		m.Close()
		p.Close()
	}
